...
Due to the absence of a magnetic field (which breaks time-reversal symmetry in the quantum Hall effect), the edge states always come in counter-propagating pairs, shown in Fig.~\ref{fig:compare_insulators}.
...
In turn, this suppresses the induced superconducting gap $\Delta_\textrm{ind}$ locally and creates unwanted subgap states in the superconductor.
...
So, to properly simulate even the thin layer of superconducting material on top of a nanowire, the lattice constant has to be smaller than an ångström, resulting in matrix sizes that go far beyond the current capabilities of even the largest computers.
