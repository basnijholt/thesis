...
Using symmetry-breaking theory, we can understand how many of the different phases arise.
It explains that these different phases correspond to different symmetries in the way that the atoms are structured.
If the phase of a material changes (called a phase transition) such a symmetry changes.
For example, in a liquid, the atoms are randomly distributed.
When moving the atoms by an arbitrary distance it stays a liquid.
...
Only when moving the atoms by an exact integer number of the lattice constant (the distance between the smallest repeating pattern), the crystal stays the same.
...
\co{To understand the topological protection of MBS in hybrid structures, we discuss superconductivity.} % XXX: rewrite this paragraph
...
Upon reaching this temperature, the material goes through a phase transition resulting in an exact zero electrical resistance and the expulsion of magnetic fields.
...
Both the phenomenological Ginzburg-Landau theory and the microscopic BCS theory led to a good understanding of ``conventional'' superconductivity.
