...
The next sections introduce superconductivity and the topological protection of Majoranas (Sec.~\ref{sec:superconductivity}), non-Abelian statistics (Sec.~\ref{sec:braiding}), the minimal hybrid Majorana model (Sec.~\ref{sec:?}), and finally, a few extensions to this model (Sec.~\ref{sec:?}).
...
Symmetry and its resulting topology is the topic of the next section, after which, we will disscus this particle-hole symmetry.
...
Subsequently, in Sec.~\ref{sec:MBS} we show that it is possible to create Majoranas at two opposite edges of a nanowire where they do not couple and are pinned to $E=0$; this is the so-called topological protection.
...
\section{Creating Majoranas in a minimal hybrid nanowire}\label{sec:MBS}
\co{We can create a topological nanowire using superconductivity, spin-orbit coupling, and a Zeeman field.}
...
This invariant can only assume $Q=+1$ (no Majoranas) and $Q=-1$ (Majoranas present) and changes when the bandgap is closes and reopens.
...
\subsection{The Hamiltonian, term by term.}
\co{We know that to create Majoranas, we have to close and reopen the bandgap. Therfore, we engineer our Hamiltonian term by term and start simple.}
...
\co{We require a bandgap, so we add superconductivity, resulting in a BdG Hamiltonian with a gapped spectrum.}
...
Now we have a gapped spectrum; however, we closed the band structure twice because of the doubly degenerate spin bands both crossing zero energy simultaneously.
We are now left in a state where for every eigenstate, there is at least one more eigenstate with the same energy, this is called Kramers degeneracy, and it needs to be broken in order to make Majoranas.
\footnote{With Kramers degeneracy, we would make two Majoranas per edge, which is just a fermion.}
...
In Fig.~\ref{fig:zeeman} we observe the effect of a magnetic field on the band structure.
\begin{figure}
\begin{center}
...
\end{center}
\end{figure}
We see that a magnetic field breaks the Kramers degeneracy.
...
\co{To break spin-rotation symmetry, we introduce the spin-orbit coupling, which by itself isn't enough to create Majoranas.}
...
\end{equation}
Spin-orbit by itself---eventhough it couples spin---is not sufficient to break the Kramers degeneracy.
