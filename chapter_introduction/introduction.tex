...
The simple one-dimensional model we introduced in the previous section is great because it can be solved analytically and therefore, gives it can give us a lot of insights.
However, because of its simplicity it also ignores many relavant physical effects.
For example, the model assumes a one-dimensional nanowire even though we live in a three-dimensional space, and so does the nanowire device when an experimentalist realizes such a device.
Besides the additional orbitals that are present in a three-dimensional system, the magnetic flux penetrating the nanowire cross-section modifies the Hamiltonian and changes the complex phases of the quasiparticles.
Futher, the simple model assumes a tunable but constant chemical potential $\mu$ inside the nanowire.
In an experiment the nanowire is close to a metal gate at a certain voltage, its electric field changes the electrostatic environment inside the nanowire, except this does not result in a perfectly homogeneous potential profile.
Another essential effect that the simple model excludes, is the effect of the electric field of the metal gates close to the nanowire.
Soft gap, no quantized conductance, we know that we are ignoring effects 
...
\co{A real system's material is not perfectly smooth and has impurities, we can model this disorder by random onsite energy.}
\co{Other improvements include a better model for proximity SC, more accurate SOC with k.p, and including self-consistent electrostatics.}
