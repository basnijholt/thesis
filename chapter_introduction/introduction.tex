...
Because there are more bands, multiple Majoranas can be created on one edge of the nanowire as $\mu$ increases; however, these additional Majoranas are not topologically protected and a small perturbation can pair-wise annihilate all but $N \mod 2$ of them.
The position of the phase boundaries now also depends on the cross-section's geometry.
For example, Fig.~\ref{fig:topo_bands} shows phase diagrams and band structures at different combinations of $B,\; \mu$ for a nanowire with a hexagonal cross-section.
\begin{figure}
\begin{center}
\includegraphics[width=0.8\textwidth]{chapter_introduction/figures/phase_diagrams_bands.pdf}
\caption{Topological phase diagram (left) of a 3D system with a blue dot that indicates the value of $B,\; \mu$ at which the band structures (right) are plotted.
