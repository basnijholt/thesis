% XXX: Fix {Note1} and references to "Supplemental Material".
...
By fitting the curvature of the gap closing \cite{Heck2017,Pan2019} along $x$ [white dashed line in Fig.~\ref{fig:fig1}(e)] we estimate a range of the SOI strength $\alpha$ of 0.15 -- 0.35 eV\AA\ from devices $A$-$D$ (for fitting details and fits to additional devices, see Supplemental Material \cite{Note1}).  % XXX: fix Note1
...
(a) Measured $dI/dV$ as a function of $V$ upon rotation of $B$ at 0.3 T over angles $\Theta$ between $z$ and $y$ in device $B$ (see Fig.~\ref{fig:YZrotRep} \cite{Note1} for the same behavior in device $A$).  % XXX: fix Note1
...
We define the angle at which the gap is hardest as $\Theta_{\mathrm{max}}$ and find $\Theta_{\mathrm{max}}$ $=$ 3 $\pm$ \ang{2} ($z$ axis) for all $V_{\mathrm{SG}}$ and in multiple devices (Fig.~\ref{fig:fig3} and Fig.~\ref{fig:YZrotRep} \cite{Note1}) (error due to uncertainty in the extraction procedure).  % XXX: fix Note1
